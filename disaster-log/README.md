...
상황전파 수신대상 연락망(부서명·직위·이름·전화번호·이메일·비고). 개별 등록/수정/삭제, **엑셀 양식 다운로드**(「연락처」+「작성안내」 시트), **엑셀 일괄 업로드**(xlsx/xls/csv · 헤더 별칭 자동 인식 · 미리보기·검증 · 추가/병합/전체교체), 내보내기. SheetJS(`xlsx`) 사용, `src/lib/org/excel.ts`. 저장은 localStorage(`contacts`).
