...
산불(경북 안동·의성) 실제재난 시나리오를 14단계로 자동 시연한다. 모달이 아닌 **우하단 플로팅 패널**(접기·좌/우 도킹·자동 진행·Alt+←/→)로 화면을 제어하며, 각 단계가 화면 이동과 데이터 조작(상황 등록 → 문서·조치 → SOP → 실행 → 상황일지 → 결과보고)을 수행하고 발표용 내레이션·포인트·「직접 해보기」를 보여준다. 시나리오 정의는 `src/lib/demo/scenario.ts`, 상태는 `src/store/useDemoStore.ts`(sessionStorage), 패널은 `src/components/demo/DemoPanel.tsx`. 마지막 단계에서 시연 상황을 삭제하고 종료할 수 있다.

## 실행·조치결과 탭 (S09)

- **목록 / 플로우** 두 보기 모두에서 조치를 클릭해 우측에서 처리한다. 플로우 보기는 실행 상태(진행 중·완료·생략)와 현재 조치를 캔버스에 표시한다.
- **시작·종료 안내창**: 실행 시작(확인 모달 → 시작 안내), 종료 노드 도달(완료 안내 · 완료/생략/SMS·자원 요약 → 상황일지로), 중지(확인 모달).
- **세부행동 체크**: 매뉴얼 세부행동을 체크박스로 수행 기록(체크 시각 · 원장 「세부행동 수행」). 조치결과를 비워 두고 완료하면 체크 항목이 조치결과로 자동 정리된다.
- **도움말(?)**: 기준 SOP·타임라인·상황 변화 대응·세부행동·분기값 등 용어에 마우스 오버 툴팁(`Help` 컴포넌트, `src/components/ui/index.tsx`).
- **상황전파 SMS**: 설정 › 조직·연락처에서 부서·담당자를 골라 수신대상에 추가.

## 실행 플로우 보기 · 노드/연결선 효과

- 실행·조치결과 탭의 플로우 보기는 전체 맞춤이 아니라 **현재 조치(시작 전에는 시작 노드)를 기준으로 1.25배 확대**해 한 화면에 2~3개 노드가 보이도록 하고, 조치를 완료하면 다음 노드가 **자동으로 화면 중앙(약간 위)** 으로 이동한다(`SopCanvas` `focusNodeId`/`focusZoom`). 캔버스 높이는 화면 높이에 맞춰 늘어난다(최소 640px). 편집 화면은 기존 전체 맞춤 유지.
- 현재 노드: 초록 외곽선 펄스(깜박임) + 뒤 빛 번짐(블러 halo). 완료 노드: 초록 톤. 생략 노드: 흐림.
- 연결선(`SopEdgeView`): 대기 회색 2.4px · 완료 초록 3px · 진행 중(완료→진행) 초록 3.5px + 흐르는 대시 + 빛 번짐 + 경로를 따라 움직이는 점 2개(SVG animateMotion). 상황판단 분기값은 연결선 라벨로 표시.

## 조직·연락처 관리 (설정 › `/settings/org`)

부산광역시 풍수해 재난 현장조치 행동매뉴얼(26.7.)의 재난안전대책본부 편성을 그대로 따른다.

- **구분 → 실무반·기관 → 부서 → 사람(직위)** 3계층: 지휘부(본부장·차장·총괄조정관·통제관·담당관) · 13개 협업기능별 실무반(재난상황관리반 … 재난수습홍보반, 붙임 8-5-①) · 유관기관(기상청·53사단·한전·KT·소방·경찰 등).
- 조치의 **주관부서·지원부서·협업기관은 조치별 역할**(행동요령 카드)이므로 조직 속성이 아니라 SOP 노드에 기록되고, 상황전파 발송 시 「조치 담당 추천」으로 해당 부서 인원을 모아 보여 준다.
- 탭: **조직도**(트리·부서별 추가) · **연락처 목록**(표·검색·선택 삭제) · **전송그룹**(주소록 그룹 CRUD, 조직도에서 부서·실무반 단위 체크).
- **엑셀 양식 다운로드 / 일괄 업로드**: 컬럼 `구분 · 실무반/기관 · 부서명 · 직위 · 이름 · 전화번호 · 이메일 · 비고`. 헤더 별칭 자동 인식, 구분·실무반은 병합셀처럼 첫 행만 적어도 이어받음, 미리보기·검증 후 추가/병합/전체교체. SheetJS(`xlsx`), `src/lib/org/excel.ts`.
- 기본 Seed(`src/lib/seed/contacts.ts`): 매뉴얼 편성 기준 가상 인물 40명 + 전송그룹 5개(재대본 지휘부 · 13개 협업기능반 반장 · 재난상황관리반 전원 · 호우 지원부서 · 유관기관 상황실).

## 상황전파 → 현장 응답 (모바일 웹, 인증 없음)

- SOP 노드에 **전파 채널(SMS·이메일)** 을 지정하고, 실행 중 「상황전파 발송」 모달에서 채널·제목·내용을 확인하고 **사람 또는 전송그룹**을 선택해 발송한다(이번 범위는 모의 발송). 우측에 **모바일 페이지 미리보기**가 있다.
- 발송 시 수신자마다 토큰 링크 `/m/{token}?p=…` 와 **QR** 이 만들어진다. payload(상황·조치·제목·내용·수신자)는 링크에 base64url 로 담겨 서버 없이도 열리고, `/api/dispatch` 에도 등록된다.
- 모바일 페이지(`src/app/m/[token]`, `MobileDispatchView`): 상황 · 조치 · 내용 · 수신자 · **① 수신확인 → ② 임무완료 → ③ 조치사항(선택)**. 큰 버튼, 반응형, 갤럭시+크롬 기준(iOS 사파리 표준 CSS). 응답은 `/api/dispatch/{token}` POST + BroadcastChannel/localStorage(같은 브라우저 시연용)로 전달된다.
- 상황실(실행 탭)은 `useDispatchSync` 로 미완료 수신자가 있을 때 4초 폴링(`/api/dispatch/pull`)하며, 응답은 **조치 카드의 응답 현황 · 실제 수행 타임라인 · 원장(상황일지) · NodeRun.missionAck · 결과보고 붙임 1-1** 에 자동 반영된다. 조치사항은 「확인 필요」로 들어온다.
- 서버 저장소(`src/lib/dispatch/server-store.ts`): 기본 프로세스 메모리. **Vercel 등 서버리스에서 다른 기기 응답을 안정적으로 받으려면 Upstash Redis(REST) 환경변수 `UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN` 을 등록**(무료 티어)하면 자동 전환된다. 미등록 시 인스턴스가 바뀌면 응답이 유실될 수 있다(같은 브라우저 시연은 영향 없음).

## SOP 편집 속성 패널

SOP 구성·편집 탭과 라이브러리 편집기의 우측 속성 패널은 **숨기기/열기** 버튼과 **왼쪽 가장자리 드래그로 폭 조절**(280~720px, 더블클릭 시 기본 360px)을 지원한다. 폭·숨김 상태는 localStorage 에 기억된다(`src/components/sop/SidePanel.tsx`).
...
- **HWPX 스타일 매핑 주의**: 한컴오피스는 `charPr`/`paraPr` 를 id 가 아닌 **목록 순서**로 해석한다. 새 모양(굵은 본문 charPr 14, 내어쓰기 paraPr 22·23·24)은 header.xml 목록 **끝에** 추가해야 하며, 중간 삽입 시 제목·부제·장제목 서식이 한 칸씩 어긋난다. □/○/- 단계는 paraPr 22/23/24 의 `left`/`intent`(내어쓰기)로 들여쓰기하므로 줄이 넘어가도 본문 시작점이 정렬된다.
